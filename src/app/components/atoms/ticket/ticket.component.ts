...
import { Observable, debounceTime, fromEvent, repeat, skipUntil, takeUntil } from 'rxjs';
...
export class TicketComponent implements AfterViewInit {
  ...
  constructor(private workflow: WorkflowPageComponent, private workflowSection: WorkflowSectionComponent) { }
  ...
  initializeObservables(){
    this.down$ = fromEvent<MouseEvent>(this.ticketElement.nativeElement,'mousedown');
    this.move$ = fromEvent<MouseEvent>(document, 'mousemove');
    this.up$ = fromEvent<MouseEvent>(this.ticketElement.nativeElement, 'mouseup');
    this.windowResize$ = fromEvent<Event>(window, 'resize').pipe(debounceTime(100));
  }

  watchForCardDrop(){
    this.up$.subscribe((event) => {
      this.workflow.cardDrop$.next({ clientX: event.clientX, clientY: event.clientY, order: this.ticket });
  ...
  watchForCardMove(){
    this.move$.pipe(
      ...
      this.workflow.moveCard$.next({ clientX: event.clientX, clientY: event.clientY, order: this.ticket });
  ...
  watchForSectionScroll(){
    this.workflowSection.cardContainerScrollTop$.pipe(
      takeUntil(this.down$)
    ).subscribe((event) => {
      if(!event) return;
